...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    K_gain = p_cov_check @ h_jac.T @ np.linalg.inv((h_jac @ p_cov_check @h_jac.T)+R)

    # 3.2 Compute error state
    delta_state = K_gain @ (y_k-p_check)

    # 3.3 Correct predicted state
    delta_p = delta_state[0:3]
    delta_v = delta_state[3:6]
    delta_q = delta_state[6:9]
    
    p_check = p_check + delta_p
    v_check = v_check + delta_v
    q_check = Quaternion(euler=delta_q).quat_mult_right(q_check)

    # 3.4 Compute corrected covariance
    p_cov_check = (np.eye(9) - K_gain@h_jac)@p_cov_check
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    sensor_rotation_matrix = Quaternion(*q_est[k-1]).to_mat()
    
    # 1. Update state with IMU inputs
    p_check = p_est[k-1] + delta_t * v_est[k-1] + ((delta_t**2)/2)*((sensor_rotation_matrix@imu_f.data[k-1])-g)
    v_check = v_est[k-1]+delta_t*((sensor_rotation_matrix@imu_f.data[k-1])-g)
    ...
    F_k[3:6,6:9] = -(sensor_rotation_matrix.dot(skew_symmetric(imu_f.data[k-1].reshape((3)))))
    # 2. Propagate uncertainty
    p_cov_check = F_k@p_cov[k-1]@F_k.T + l_jac@Q_k@l_jac.T

    # 3. Check availability of GNSS and LIDAR measurements
    if gnss_i < gnss.t.shape[0] and imu_f.t[k-1] == gnss.t[gnss_i]:
        p_check, v_check, q_check, p_cov_check = measurement_update(var_gnss, p_cov_check, gnss.data[gnss_i].T,p_check,v_check, q_check)
        gnss_i += 1
    
    if lidar_i < lidar.t.shape[0] and imu_f.t[k-1]== lidar.t[lidar_i]:
        p_check, v_check, q_check, p_cov_check = measurement_update(var_lidar, p_cov_check, lidar.data[lidar_i].T,p_check,v_check, q_check)
        lidar_i += 1
